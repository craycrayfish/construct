...
async def test_on_step_callback_invoked(construct_config, mock_vlm, mock_odyssey_session):
...
async def test_on_step_none_is_fine(construct_config, mock_vlm, mock_odyssey_session):
...
def test_make_frame_saver_writes_pngs(tmp_path):
    """_make_frame_saver should write PNG files named step_NNNN.png."""
    ...
    assert (frames_dir / "step_0000.png").exists()
    assert (frames_dir / "step_0001.png").exists()
    # Verify it's valid PNG (starts with PNG signature)
    data = (frames_dir / "step_0000.png").read_bytes()
...
def test_run_output_dir_structure(tmp_path, construct_config, mock_vlm, mock_odyssey_session, monkeypatch):
    ...
    with (
        patch("construct.runner.OdysseySession", return_value=session),
        patch("construct.cli.ConstructConfig", return_value=construct_config),
        patch("construct.cli.GeminiRoboticsBackend", return_value=vlm),
    ):
...
def test_no_frames_skips_directory(tmp_path, construct_config, mock_vlm, mock_odyssey_session, monkeypatch):
...
def test_json_output(tmp_path, construct_config, mock_vlm, mock_odyssey_session, monkeypatch, capsys):
